...
class Model:
    ...
    def __init__(self):
        ...
        for x in range(64):
            for z in range(64):
                ...
                for i in range(3,7): self.cubes.add((x,-i-y,-z),'stone')
    ...
    def addTree(self,pos):
        x,y,z = pos
        for Y in (0,1,2,3,4,5): self.cubes.add((x,y+Y,z),'log_oak')
        for X in (-2,-1,0,1,2):
            for Z in (-2,-1,0,1,2):
                if X==2 and Z==2: continue
                for Y in (3,4): self.cubes.add((x+X,y+Y,z+Z),'leaves_oak')
        for X,Z in ((-1,0),(1,0),(0,-1),(0,1)):
            for Y in (5,6): self.cubes.add((x+X,y+Y,z+Z),'leaves_oak')
        for X,Y,Z in ((0,6,0),(-1,5,-1),(1,5,-1)): self.cubes.add((x+X,y+Y,z+Z),'leaves_oak')
